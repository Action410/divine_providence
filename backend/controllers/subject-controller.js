...
const subjectCreate = async (req, res) => {
    try {
        if (!req.body.subjects || !Array.isArray(req.body.subjects) || req.body.subjects.length === 0) {
            return res.status(400).send({ message: 'Subjects array is required and cannot be empty' });
        }
        ...
        // Check for duplicate subCodes in the request
        const subCodes = subjects.map(s => s.subCode);
        const uniqueSubCodes = new Set(subCodes);
        if (subCodes.length !== uniqueSubCodes.size) {
            return res.status(400).send({ message: 'Duplicate subject codes are not allowed' });
        }

        // Check if any subCode already exists in the database
...
const deleteSubjects = async (req, res) => {
    try {
        // Find subjects before deleting to get their IDs
